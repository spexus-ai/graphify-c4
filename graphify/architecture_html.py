...
let currentNodes = [], currentRelations = [], hiddenNodes = new Set(), pinnedNodeId = null, cameraTargetId = null;
...
function focusNode(id, scale=1.55) { if(!nodesDS.get(id))return; network.selectNodes([id]);network.focus(id,{scale,animation:{duration:650,easingFunction:'easeInOutQuad'}});showInfo(id); }
function focusAfterRender() { const id=cameraTargetId;cameraTargetId=null;if(id)requestAnimationFrame(()=>focusNode(id)); }
...
network.on('click',params=>{if(params.nodes.length){const nodeId=params.nodes[0];if(pinnedNodeId!==nodeId){pinnedNodeId=nodeId;cameraTargetId=nodeId;updateGraph();}else focusNode(nodeId);}else info.innerHTML='<span class="empty">Click a node to inspect it</span>';});network.on('doubleClick',params=>{if(params.nodes.length)drill(params.nodes[0]);});
document.addEventListener('click',event=>{const link=event.target.closest('.neighbor-link');if(link)focusNode(link.dataset.nid);});
search.addEventListener('input',()=>{const query=search.value.toLowerCase().trim();results.innerHTML='';if(!query){results.style.display='none';return;}const matches=currentNodes.filter(item=>!hiddenNodes.has(item.id)&&item.name.toLowerCase().includes(query)).slice(0,20);if(!matches.length){results.style.display='none';return;}results.style.display='block';for(const item of matches){const entry=document.createElement('div'),style=visual(item);entry.className='search-item';entry.textContent=item.name;entry.style.borderLeft=`3px solid ${style.color}`;entry.onclick=()=>{focusNode(item.id);results.style.display='none';search.value='';};results.append(entry);}});
selectAll.addEventListener('change',()=>{hiddenNodes=selectAll.checked?new Set():new Set(currentNodes.map(item=>item.id));applyNodeFilter();renderNodeFilter();});level.onchange=()=>{pinnedNodeId=null;updateGraph();};focus.onchange=()=>{pinnedNodeId=null;cameraTargetId=focus.value||null;updateGraph();};document.querySelector('#reset').onclick=()=>{focus.value='';pinnedNodeId=null;updateGraph();};renderFocus();updateGraph();
</script></body></html>"""
