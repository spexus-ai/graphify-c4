...
function chooseSubgraph(elements, relations, pinnedId) { if(elements.length<=maxGraphNodes)return {elements,relations,trimmed:false}; const score=new Map(elements.map(item=>[item.id,0])); for(const relation of relations){score.set(relation.source,(score.get(relation.source)||0)+relation.evidence_count);score.set(relation.target,(score.get(relation.target)||0)+relation.evidence_count);} const ranked=[...elements].sort((a,b)=>(score.get(b.id)-score.get(a.id))||a.id.localeCompare(b.id)); const ids=new Set; if(pinnedId&&score.has(pinnedId)){ids.add(pinnedId); for(const relation of relations.filter(item=>item.source===pinnedId||item.target===pinnedId).sort((a,b)=>b.evidence_count-a.evidence_count)){if(ids.size>=maxGraphNodes)break;ids.add(relation.source);ids.add(relation.target);}} for(const item of ranked){if(ids.size>=maxGraphNodes)break;ids.add(item.id);} return {elements:elements.filter(item=>ids.has(item.id)),relations:relations.filter(relation=>ids.has(relation.source)&&ids.has(relation.target)),trimmed:true}; }
