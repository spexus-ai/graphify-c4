...
const maxGraphNodes = 180, maxFocusedCodeNodes = 750, byId = new Map(payload.elements.map(item => [item.id,item]));
...
function chooseSubgraph(elements, relations, pinnedId, maxNodes) { if(elements.length<=maxNodes)return {elements,relations,trimmed:false,bridgeCount:0}; const score=new Map(elements.map(item=>[item.id,0])); for(const relation of relations){const weight=relation.evidence_count+(relation.bridge?4:0);score.set(relation.source,(score.get(relation.source)||0)+weight);score.set(relation.target,(score.get(relation.target)||0)+weight);} const ranked=[...elements].sort((a,b)=>(score.get(b.id)-score.get(a.id))||a.id.localeCompare(b.id)),bridgeRank=(a,b)=>((score.get(b.source)+score.get(b.target))-(score.get(a.source)+score.get(a.target)))||b.evidence_count-a.evidence_count||a.source.localeCompare(b.source); const ids=new Set, add=id=>{if(ids.size<maxNodes)ids.add(id);}; if(pinnedId&&score.has(pinnedId)){add(pinnedId); for(const relation of relations.filter(item=>item.source===pinnedId||item.target===pinnedId).sort(bridgeRank)){add(relation.source);add(relation.target);}} else { for(const item of ranked.slice(0,Math.floor(maxNodes*.7)))add(item.id); for(const relation of relations.filter(item=>item.bridge&&(ids.has(item.source)||ids.has(item.target))).sort(bridgeRank)){add(relation.source);add(relation.target);} } for(const item of ranked)add(item.id); const keptRelations=relations.filter(relation=>ids.has(relation.source)&&ids.has(relation.target)); return {elements:elements.filter(item=>ids.has(item.id)),relations:keptRelations,trimmed:true,bridgeCount:keptRelations.filter(item=>item.bridge).length}; }
