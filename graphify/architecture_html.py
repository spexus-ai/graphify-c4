...
<div id="sidebar">
  <div id="controls">
    ...
    <div id="mode-note">Drag nodes to arrange them. Scroll to zoom. Double-click a C4 node to drill down.</div>
  </div>
  ...
  <div id="legend-wrap"><h3>Relationship evidence</h3><div class="edge-key"><i></i>Declared contract</div><div class="edge-key"><i class="observed"></i>Observed code evidence</div><h3 style="margin-top:18px">Visible nodes</h3><div id="legend-controls"><label><input type="checkbox" id="select-all-cb" checked> Select All</label></div><div id="node-filter"></div></div>
  <div id="stats"></div>
</div>
<script>
...
const maxGraphNodes = 180, byId = new Map(payload.elements.map(item => [item.id,item]));
const level = document.querySelector('#level'), focus = document.querySelector('#focus'), info = document.querySelector('#info-content'), stats = document.querySelector('#stats'), search = document.querySelector('#search'), results = document.querySelector('#search-results'), nodeFilter = document.querySelector('#node-filter'), selectAll = document.querySelector('#select-all-cb');
...
let currentNodes = [], currentRelations = [], hiddenNodes = new Set();
...
function renderNodeFilter() { nodeFilter.innerHTML=''; for(const item of currentNodes){const row=document.createElement('div'),checkbox=document.createElement('input'),dot=document.createElement('div'),label=document.createElement('span');row.className='legend-item';checkbox.type='checkbox';checkbox.className='legend-cb';checkbox.checked=!hiddenNodes.has(item.id);dot.className='legend-dot';dot.style.background=nodeColors[item.c4_type]||'#4E79A7';label.className='legend-label';label.textContent=item.name;checkbox.onchange=()=>{if(checkbox.checked)hiddenNodes.delete(item.id);else hiddenNodes.add(item.id);row.classList.toggle('dimmed',!checkbox.checked);applyNodeFilter();};row.append(checkbox,dot,label);row.onclick=event=>{if(event.target===checkbox)return;checkbox.checked=!checkbox.checked;checkbox.dispatchEvent(new Event('change'));};nodeFilter.append(row);} updateSelectAllState(); }
function updateGraph() { const allElements=visible(), allRelations=relationships(allElements), subset=chooseSubgraph(allElements,allRelations); currentNodes=subset.elements;currentRelations=subset.relations;hiddenNodes=new Set(); const degree=new Map(currentNodes.map(item=>[item.id,0]));for(const relation of currentRelations){degree.set(relation.source,(degree.get(relation.source)||0)+1);degree.set(relation.target,(degree.get(relation.target)||0)+1);} const maxDegree=Math.max(...degree.values(),1); nodesDS.clear();edgesDS.clear();nodesDS.add(currentNodes.map(item=>({id:item.id,label:item.name,color:{background:nodeColors[item.c4_type]||'#4E79A7',border:nodeColors[item.c4_type]||'#4E79A7',highlight:{background:'#ffffff',border:nodeColors[item.c4_type]||'#4E79A7'}},size:10+30*(degree.get(item.id)/maxDegree),font:{size:degree.get(item.id)>=maxDegree*.15?12:0,color:'#ffffff'},title:esc(`${item.name}\n${item.c4_type}\n${item.source_file||item.id}`),_item:item,_degree:degree.get(item.id)}))); edgesDS.add(currentRelations.map((relation,index)=>({id:index,from:relation.source,to:relation.target,label:'',title:esc(`${relation.kind} (${relation.origin})${relation.evidence_count?` — ${relation.evidence_count} evidence`:''}`),dashes:relation.origin==='observed',width:relation.origin==='declared'?2:1,color:{color:relation.origin==='declared'?'#22c55e':'#f59e0b',opacity:relation.origin==='declared'?.85:.7},arrows:{to:{enabled:true,scaleFactor:.5}},_relation:relation}))); renderNodeFilter(); info.innerHTML='<span class="empty">Click a node to inspect it</span>'; results.style.display='none'; search.value=''; stats.innerHTML=`${allElements.length} ${esc(level.value)} nodes &middot; ${allRelations.length} relationships${subset.trimmed?` &middot; showing ${subset.elements.length} most connected`:''}`; network.setOptions({physics:{enabled:true}});network.stabilize(200);network.once('stabilizationIterationsDone',()=>network.setOptions({physics:{enabled:false}})); }
function showInfo(nodeId) { const node=nodesDS.get(nodeId);if(!node)return;const item=node._item,neighbors=network.getConnectedNodes(nodeId),neighborItems=neighbors.map(id=>{const neighbor=nodesDS.get(id);return `<span class="neighbor-link" style="border-left-color:${esc(neighbor.color.background)}" data-nid="${esc(id)}">${esc(neighbor.label)}</span>`;}).join(''),next=nextLevel(item.c4_type); info.innerHTML=`<div class="field"><b>${esc(item.name)}</b></div><div class="field">Type: ${esc(item.c4_type)}</div><div class="field">Source: ${esc(item.source_file||'declared')}</div><div class="field">Degree: ${node._degree}</div>${next?'<button class="drill-button" id="drill">Drill down</button>':''}${neighbors.length?`<div class="field" style="margin-top:8px;color:#aaa;font-size:11px">Neighbors (${neighbors.length})</div><div id="neighbors-list">${neighborItems}</div>`:''}`; const button=document.querySelector('#drill');if(button)button.onclick=()=>drill(item.id); }
...
network.on('click',params=>{if(params.nodes.length)showInfo(params.nodes[0]);else info.innerHTML='<span class="empty">Click a node to inspect it</span>';});network.on('doubleClick',params=>{if(params.nodes.length)drill(params.nodes[0]);});
document.addEventListener('click',event=>{const link=event.target.closest('.neighbor-link');if(link)showInfo(link.dataset.nid);});
search.addEventListener('input',()=>{const query=search.value.toLowerCase().trim();results.innerHTML='';if(!query){results.style.display='none';return;}const matches=currentNodes.filter(item=>item.name.toLowerCase().includes(query)).slice(0,20);if(!matches.length){results.style.display='none';return;}results.style.display='block';for(const item of matches){const entry=document.createElement('div');entry.className='search-item';entry.textContent=item.name;entry.style.borderLeft=`3px solid ${nodeColors[item.c4_type]||'#4E79A7'}`;entry.onclick=()=>{network.focus(item.id,{scale:1.5,animation:true});network.selectNodes([item.id]);showInfo(item.id);results.style.display='none';search.value='';};results.append(entry);}});
selectAll.addEventListener('change',()=>{hiddenNodes=selectAll.checked?new Set():new Set(currentNodes.map(item=>item.id));applyNodeFilter();renderNodeFilter();});level.onchange=updateGraph;focus.onchange=updateGraph;document.querySelector('#reset').onclick=()=>{focus.value='';updateGraph();};renderFocus();updateGraph();
</script></body></html>"""
